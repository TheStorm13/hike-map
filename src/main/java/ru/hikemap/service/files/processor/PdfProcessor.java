...
public class PdfProcessor implements FileProcessor {
  ...
  public String processFile(Long hikeId, MultipartFile file) throws Exception {
    // Проверить тип файла (например, MIME-тип)
    ...
    hike.setReportPdfPath(fileName);
  ...
  public Resource loadFile(Long hikeId) {
    ...
    // Получение пути к файлу PDF, сохранённого для похода
    String fileName = hike.getReportPdfPath();

    if (fileName == null || fileName.isEmpty()) {
      throw new IllegalStateException(
        "No PDF file associated with Hike ID: " + hikeId
      );
    }
  ...
  public void removeFile(Long hikeId) {
    ...
    if (fileName == null || fileName.isEmpty()) {
      throw new IllegalStateException(
        "No PDF file to remove for Hike ID: " + hikeId
      );
    }
